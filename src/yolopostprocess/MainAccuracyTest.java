...
//		picIds.remove("1479498419466839188");
		...
		Map<String, YoloFinding> yoloFindings = YoloUtil.getYoloFindings(
		...
		List<Knob> knobList = getAllKnobs();
		Map<Knob, Map<String, Resource>> knobToPicIdToResource =
				getKnobToPicIdToResource(knobList, usefulPicIds, 
						groundTruthFindings, yoloFindings);
		Map<Knob, Map<String, EvalStats>> knobToPicIdToEvalStats =
				getKnobToPicIdToEvalStats(knobList, usefulPicIds, 
						groundTruthFindings, yoloFindings);
//		PrecesionRecallAnalyze(usefulPicIds, groundTruthFindings, yoloFindings);
		PrecesionRecallAnalyze(usefulPicIds, knobList, 
				knobToPicIdToResource, knobToPicIdToEvalStats);
	}
	
	private static void PrecesionRecallAnalyze(Set<String> picIds, 
			List<Knob> knobList,
			Map<Knob, Map<String, Resource>> knobToPicIdToResource,
			Map<Knob, Map<String, EvalStats>> knobToPicIdToEvalStats) {
		for (String picId : picIds){		
...
//					double utility = fsize * (1-fscore);
	...
	private static List<Knob> getAllKnobs() {
	...
	private static Map<Knob, Map<String, Resource>> getKnobToPicIdToResource(
			List<Knob> knobList, Set<String> picIds,
		...
		return knobToPicIdToResource;
	}
	private static Map<Knob, Map<String, EvalStats>> getKnobToPicIdToEvalStats(
			List<Knob> knobList, Set<String> picIds,
			Map<String, Finding> groundTruthFindings, 
			Map<String, YoloFinding> yoloFindings) {
		Map<Knob, Map<String, EvalStats>> knobToPicIdToEvalStats = 
